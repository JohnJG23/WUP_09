...
when we use anavo it comes up with a p value that is .03 and is comparing both models and since the p value is lower than . 05 we conclude that it is significant below the .05 level.  
...
the roach best model description
...
were comparing the best roch model with the five element roach model, the comparison results in a p value of .9, the rss in the 5 element model is slightly smaller but not a significant diffrence, anova says comparing these two modeles togther results in no statistic difrence. 
